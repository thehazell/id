...
export async function userinfo(
    ...
    const scopes = new Set(
        accessToken.scope.split(" ").filter(Boolean),
    );
    ...
    if (accessToken.authorizationCodeId) {
        ...
        if (requestedClaims) {
            try {
                ...
                for (const claim of Object.keys(
                    parsed.userinfo ?? {},
                )) {
    ...
    const definitions =
        getUserInfoClaimDefinitions();

    const claims: Record<string, unknown> = {
        sub: user.id,
    };

    for (const [claimName, definition] of Object.entries(
        definitions,
    )) {
        ...
        const explicitlyRequested =
            requestedUserInfoClaims.has(claimName);

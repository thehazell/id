...
function getAuthorizationRequestFromQuery(
    ...
    const queryCodeChallengeMethod = c.req.query(
    ...
    const redirectUri =
        requestClaims.redirect_uri ?? queryRedirectUri;
    const responseType =
        requestClaims.response_type ?? queryResponseType;
    ...
    const codeChallengeMethod =
        requestClaims.code_challenge_method ??
...
route.get("/", async (c) => {
    try {
        ...
        const validation =
            await validateAuthorizationRequest(db, request);
        ...
        const authenticationAge = sessionRecord
            ? Math.floor(Date.now() / 1000) -
              Math.floor(
                  sessionRecord.session.createdAt / 1000,
              )
            : null;

        const maxAgeExpired =
            maxAge !== undefined &&
            (authenticationAge === null ||
                authenticationAge >= Number(maxAge));
